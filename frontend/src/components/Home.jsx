    ...
    return (
        <div className='my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
            ...
            <div className='flex mb-4 '>
                <input type="text" placeholder='Add a new todo' value={newTodo} onChange={(e) => setNewTodo(e.target.value)} onKeyPress={(e) => e.key === "Enter" && todoCreate()} className='grow p-2 border rounded-l-md focus:outline-none' />
                <button onClick={todoCreate} className='bg-blue-600 border border-blue-600 rounded-r-md text-white px-4 py-2 hover:bg-blue-900 duration-300'>Add</button>
            </div>
                    ...
                    {todos.map((todo, index) => (
                        <li key={todo._id || index} className='flex items-center justify-between p-3 bg-gray-100 rounded-md'>
                            <div className='flex items-center'>
                                <input type="checkbox" checked={todo.completed} onChange={() => todoStatus(todo._id)} className='mr-2 cursor-pointer' />
                                <span className={`${todo.completed ? "line-through text-gray-800 font-semibold" : ""}`}>{todo.text}</span>
                            </div>
                            <button onClick={() => todoDelete(todo._id)} className='text-red-500 hover:text-red-800 duration-300 cursor-pointer'>Delete</button>
                        </li>
                    ))}
                </ul>
            )}
